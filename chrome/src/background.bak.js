...
luminateEdit = {
  ...
  getCurrentServlet: function() {
    ...
    if(tabUrl == null || tabUrl.indexOf('/site/') == -1) {
      return null;
    }
    else {
      return tabUrl.split('/site/')[1].split('/')[0].split('?')[0];
    }
  }, 
  ...
  getQueryParam: function(paramName) {
    ...
    if(tabUrl == null || tabUrl.indexOf('?') == -1) {
      ...
      var queryStrings = '&' + tabUrl.split('?')[1];
  ...
  goToEditUrl: function() {
    ...
    chrome.tabs.query({
      ...
      tabUrl = allTabs[0].url;
      
      var currentServlet = luminateEdit.getCurrentServlet();
      if(tabUrl != null && currentServlet != null) {
        chrome.tabs.create({
          url: tabUrl.split('/site/')[0] + '/site/' + luminateEdit.servlets[currentServlet].getUrl()
        });
      }
    });
  }, 
  ...
  servlets: {
    ...
    PageNavigator: {
      getUrl: function() {
        ...
        currentPageName = tabUrl.split('.html')[0].split('/')[tabUrl.split('/').length - 1];
...
/* bind listeners */
chrome.tabs.onUpdated.addListener(luminateEdit.checkForLuminateOnlineUrl);
chrome.pageAction.onClicked.addListener(luminateEdit.goToEditUrl);